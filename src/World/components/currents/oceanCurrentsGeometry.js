...
async function createOceanCurrentsGeometry(initialPositions, shaderUniforms) {


    // definition of common uniform and attributes   
    const gpuComputeUVs = new Float32Array( shaderUniforms.uOceanMaxParticleCount.value * 2 * 4);

    let p = 0;

    var WIDTH = Math.sqrt(shaderUniforms.uOceanMaxParticleCount.value * 4)
...
//    const baseGeometry = new PlaneGeometry()
    ...
    geometry.instanceCount = shaderUniforms.uOceanParticleCount.value; 
    geometry.maxInstancedCount = shaderUniforms.uOceanMaxParticleCount.value * 4; 
...
export {createOceanCurrentsGeometry}