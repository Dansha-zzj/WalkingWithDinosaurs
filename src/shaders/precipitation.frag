...
uniform sampler2D thisPrecipitationFrame;
uniform sampler2D nextPrecipitationFrame;
uniform sampler2D colorMapPrecipitation;
...
if (uCMIP6Mode) {
    ...
    precipitationValueInt = mix(textureBicubic(thisPrecipitationFrame,coord, numLon, numLat),textureBicubic(nextPrecipitationFrame,coord, numLon, numLat),uFrameWeight);

    vec4 precipitationColor = applyColormap( precipitationValueInt.r, colorMapPrecipitation );
    ...
    //precipitationValueInt = mix(texture2D(thisPrecipitationFrame,coord),texture2D(nextPrecipitationFrame,coord),uFrameWeight);
    precipitationValueInt = mix(textureBicubic(thisPrecipitationFrame,coord, numLon, numLat),textureBicubic(nextPrecipitationFrame,coord, numLon, numLat),uFrameWeight);

    float precipitationValueExpanded = remap( precipitationValueInt.r, 0.0, 1.0, 0.0, 25.0 );
    float precipitationValueRemapped = remap( precipitationValueExpanded, uPrecipitationMinValue, uPrecipitationMaxValue, 0.0, 1.0 );

    vec4 precipitationColor = applyColormap( precipitationValueRemapped, colorMapPrecipitation );
