...
void main() {
    ...
    float duneHeight = texture2D(duneHeightFrame,vc2D).x - uMinDuneHeight;

    if (duneHeight <= 0.) {
        duneHeight = 0.0;
    }

    pos.z = height * uHeightDisplacement + 0.01 + duneHeight * uHeightDisplacementDunes * uOpacityDunes;
